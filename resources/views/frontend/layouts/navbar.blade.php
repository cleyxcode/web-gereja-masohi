        ...
        <div class="flex items-center justify-between h-[68px]">
            ...
            <a href="{{ route('home') }}" class="flex items-center gap-3 group shrink-0">
                <div class="relative flex items-center justify-center w-9 h-9">
                    <div
                        class="absolute inset-0 bg-primary rounded-xl rotate-6 opacity-20 group-hover:rotate-12 transition-transform duration-300">
                    </div>
                    <div
                        class="relative bg-primary/10 rounded-xl w-full h-full flex items-center justify-center overflow-hidden">
                ...
                @auth
                    ...
                    <div x-data="notificationManager" class="relative" @click.away="open = false">
                        ...
                        <div x-show="open" x-transition:enter="transition ease-out duration-200"
                            ...
                            {{-- Heade --}}
                            <div
                                class="flex items-center justify-between px-4 py-3.5 border-b border-gray-100 bg-gradient-to-r from-slate-50 to-white">
                            ...
                            <div class="max-h-[380px] overflow-y-auto divide-y divide-gray-50">
                                @forelse(Auth::user()->notifications()->take(15)->get() as $notif)
                                    ...
                                    <a href="{{ route('notifications.read', $notif->id) }}"
                                        ...
                                        {{-- Icon --}}
                                        <div
                                            class="flex-shrink-0 w-9 h-9 rounded-xl {{ $iconBg }} flex items-center justify-center mt-0.5 group-hover:scale-105 transition-transform duration-200">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <p
                                                class="text-[12.5px] font-semibold leading-snug mb-0.5 line-clamp-2 {{ $isUnread ? 'text-gray-900' : 'text-gray-600' }}">
                                                {{ $judul }}
                                            </p>
                                            <p class="text-[11.5px] text-gray-500 leading-snug mb-1 line-clamp-2">{{ $pesan }}
                                            </p>
                                            <div class="flex items-center gap-1.5">
                                                <span
                                                    class="material-symbols-outlined text-[11px] text-gray-300">schedule</span>
                                                <span
                                                    class="text-[10.5px] text-gray-400 font-medium">{{ $notif->created_at->diffForHumans() }}</span>
                                                @if($isUnread)
                                                    <span
                                                        class="ml-auto w-1.5 h-1.5 rounded-full bg-primary/70 flex-shrink-0"></span>
                                                @endif
                                            </div>
                                        </div>
                                        ...
                                        {{-- Arrow --}}
                                        <span
                                            class="material-symbols-outlined text-[16px] text-gray-300 group-hover:text-primary group-hover:translate-x-0.5 transition-all duration-200 mt-2 flex-shrink-0">chevron_right</span>
                                    </a>
                                @empty
                                    <div class="flex flex-col items-center justify-center py-10 px-4 text-center">
                                        <div
                                            class="w-16 h-16 rounded-2xl bg-gray-50 border border-gray-100 flex items-center justify-center mb-3">
                                            <span
                                                class="material-symbols-outlined text-[30px] text-gray-300">notifications_off</span>
                                        </div>
                                        <p class="text-[13px] font-semibold text-gray-600 mb-1">Belum ada notifikasi</p>
                                        <p class="text-[11px] text-gray-400 leading-relaxed">Notifikasi berita & jadwal ibadah
                                            baru<br>akan muncul di sini.</p>
                                    </div>
                                @endforelse
                            </div>
                    ...
                    {{-- Profile Button --}}
                    <a href="{{ route('profile') }}" class="flex items-center gap-2.5 pl-2 pr-4 py-2 rounded-xl text-sm font-semibold text-gray-700
                        ...
                        @else
                            <div
                                class="w-7 h-7 rounded-lg bg-gradient-to-br from-primary to-blue-400 flex items-center justify-center text-white text-xs font-black shrink-0 shadow-sm">
                        ...
                        <div class="flex flex-col leading-none">
                            ...
                            <span
                                class="text-[13px] text-gray-800 font-semibold">{{ Str::limit(Auth::user()->name, 14) }}</span>
                        </div>
                        <span
                            class="material-symbols-outlined text-[16px] text-gray-400 group-hover:text-gray-600 ml-1">expand_more</span>
                    </a>
                    ...
                    {{-- Logout --}}
                    <button onclick="handleLogout()" class="flex items-center gap-1.5 px-3 py-2 rounded-xl text-[13px] font-semibold
                                               text-red-500 hover:text-red-600 hover:bg-red-50
                                               border border-transparent hover:border-red-100
                                               transition-all duration-200 group">
                        <span
                            class="material-symbols-outlined text-[18px] group-hover:translate-x-0.5 transition-transform">logout</span>
                        Keluar
                    </button>

                @else
                    {{-- Tombol Login & Register untuk tamu --}}
                    <a href="{{ route('login') }}" class="flex items-center gap-1.5 px-4 py-2 rounded-xl text-[13px] font-semibold
                    ...
                    <a href="{{ route('register') }}" class="flex items-center gap-1.5 px-4 py-2 rounded-xl text-[13px] font-semibold
        ...
        <div class="px-4 pb-4 pt-2 border-t border-gray-100 flex items-center justify-between gap-3">
            ...
            @auth
                <a href="{{ route('profile') }}"
                    ...
                    @else
                        <div
                            class="w-8 h-8 rounded-lg bg-gradient-to-br from-primary to-blue-400 flex items-center justify-center text-white text-sm font-black shadow-sm">
                            {{ strtoupper(substr(Auth::user()->name, 0, 1)) }}
                        </div>
                    @endif
                    <div>
